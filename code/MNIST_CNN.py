...
Script Posted by Lucasz8000 on kaggle
"""
import pandas as pd
import gzip
import os
...
class CNN(object):
    ...
    def __setstate__(self, weights):
     #   (self.layer0.W, self.layer0.b, self.layer1.W, self.layer1.b, self.layer2.W, self.layer2.b, self.layer3.W, self.layer3.b) = state
        i = iter(weights)
	print dir(i) # for debuggings
        for p in self.params:
	    if sys.version_info[0]==3:
	        p.set_value(i.__next__())  # is __next__ a Python 3 thing? 
            else:
                p.set_value(i.next()) 
...
def run():
    ...
    dtype = np.float32
    data = np.loadtxt("../data/train.csv", dtype=dtype,
                           delimiter=',', skiprows=1)
    test_data = np.loadtxt("../data/test.csv", dtype=dtype,
                          delimiter=',', skiprows=1)
    print (data.shape)
